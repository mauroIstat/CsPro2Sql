DELIMITER ;;
CREATE PROCEDURE @SCHEMA.`proc_linkage_step_5`()
BEGIN

    INSERT INTO @SCHEMA.linkage_log (id_object) VALUES ('proc_linkage_step_5');

    DROP TABLE IF EXISTS @SCHEMA.linkage_step_5;

    CREATE TABLE @SCHEMA.linkage_step_5 AS
        SELECT
        ...
        FROM
            @SCHEMA.linkage_pes_matching_var pes JOIN @SCHEMA.linkage_cens_matching_var cens ON
                ...
                left(pes.FIXED_NAME_3, 1) = left(cens.FIXED_NAME_3, 1) AND
                pes.COD_EA = cens.COD_EA
        WHERE
            NOT pes.ID_INDIV IN (SELECT ID_INDIVIDUAL_PES FROM @SCHEMA.linked_record);

    ALTER TABLE @SCHEMA.linkage_step_5 ADD PRIMARY KEY (PES_ID, CENS_ID);
